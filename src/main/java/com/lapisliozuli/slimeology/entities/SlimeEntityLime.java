...
public class SlimeEntityLime extends SlimeEntityColoured {

    public SlimeEntityLime(EntityType<? extends SlimeEntityLime> arg, World arg2) {
        super(arg, arg2);
    }

    @Override
    protected ParticleEffect getParticles() {
        return new ItemStackParticleEffect(ParticleTypes.ITEM, new ItemStack(RegisterItems.SLIME_BALL_LIME));
    }

    public static final EntityType<SlimeEntityLime> SLIME_ENTITY_LIME = Registry.register(
            Registry.ENTITY_TYPE,
            new Identifier(Slimeology.MOD_ID, "slime_entity_lime"),
            FabricEntityTypeBuilder.<SlimeEntityLime>create(SpawnGroup.MONSTER, SlimeEntityLime::new)
                    .dimensions(EntityDimensions.changing(2.04f, 2.04f))
                    .trackable(160,4).build());

    public static final EntityType secPointer = SLIME_ENTITY_LIME;


    // Adapted from SlimeEntityColoured.canSpawnSEC()
    public static boolean canSpawnSECLime(EntityType<? extends SlimeEntityLime> type, WorldAccess world, SpawnReason spawnReason,
