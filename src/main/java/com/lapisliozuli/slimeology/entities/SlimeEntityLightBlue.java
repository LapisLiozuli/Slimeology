...
public class SlimeEntityLightBlue extends SlimeEntityColoured {

    public SlimeEntityLightBlue(EntityType<? extends SlimeEntityLightBlue> arg, World arg2) {
        super(arg, arg2);
    }

    @Override
    protected ParticleEffect getParticles() {
        return new ItemStackParticleEffect(ParticleTypes.ITEM, new ItemStack(RegisterItems.SLIME_BALL_LIGHT_BLUE));
    }

    public static final EntityType<SlimeEntityLightBlue> SLIME_ENTITY_LIGHT_BLUE = Registry.register(
            Registry.ENTITY_TYPE,
            new Identifier(Slimeology.MOD_ID, "slime_entity_light_blue"),
            FabricEntityTypeBuilder.<SlimeEntityLightBlue>create(SpawnGroup.MONSTER, SlimeEntityLightBlue::new)
                    .dimensions(EntityDimensions.changing(2.04f, 2.04f))
                    .trackable(160,4).build());

    public static final EntityType secPointer = SLIME_ENTITY_LIGHT_BLUE;


    // Adapted from SlimeEntityColoured.canSpawnSEC()
    public static boolean canSpawnSECLightBlue(EntityType<? extends SlimeEntityLightBlue> type, WorldAccess world, SpawnReason spawnReason,
