...
public class SlimeEntityOrange extends SlimeEntityColoured {

    public SlimeEntityOrange(EntityType<? extends SlimeEntityOrange> arg, World arg2) {
        super(arg, arg2);
    }

    @Override
    protected ParticleEffect getParticles() {
        return new ItemStackParticleEffect(ParticleTypes.ITEM, new ItemStack(RegisterItems.SLIME_BALL_ORANGE));
    }

    public static final EntityType<SlimeEntityOrange> SLIME_ENTITY_ORANGE = Registry.register(
            Registry.ENTITY_TYPE,
            new Identifier(Slimeology.MOD_ID, "slime_entity_orange"),
            FabricEntityTypeBuilder.<SlimeEntityOrange>create(SpawnGroup.MONSTER, SlimeEntityOrange::new)
                    .dimensions(EntityDimensions.changing(2.04f, 2.04f))
                    .trackable(160,4).build());

    public static final EntityType secPointer = SLIME_ENTITY_ORANGE;


    // Adapted from SlimeEntityColoured.canSpawnSEC()
    public static boolean canSpawnSECOrange(EntityType<? extends SlimeEntityOrange> type, WorldAccess world, SpawnReason spawnReason,
