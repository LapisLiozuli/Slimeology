...
public class SlimeEntityBrown extends SlimeEntityColoured {

    public SlimeEntityBrown(EntityType<? extends SlimeEntityBrown> arg, World arg2) {
        super(arg, arg2);
    }

    @Override
    protected ParticleEffect getParticles() {
        return new ItemStackParticleEffect(ParticleTypes.ITEM, new ItemStack(RegisterItems.SLIME_BALL_BROWN));
    }

    public static final EntityType<SlimeEntityBrown> SLIME_ENTITY_BROWN = Registry.register(
            Registry.ENTITY_TYPE,
            new Identifier(Slimeology.MOD_ID, "slime_entity_brown"),
            FabricEntityTypeBuilder.<SlimeEntityBrown>create(SpawnGroup.MONSTER, SlimeEntityBrown::new)
                    .dimensions(EntityDimensions.changing(2.04f, 2.04f))
                    .trackable(160,4).build());

    public static final EntityType secPointer = SLIME_ENTITY_BROWN;


    // Adapted from SlimeEntityColoured.canSpawnSEC()
    public static boolean canSpawnSECBrown(EntityType<? extends SlimeEntityBrown> type, WorldAccess world, SpawnReason spawnReason,
