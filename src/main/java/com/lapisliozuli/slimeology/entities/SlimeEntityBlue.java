...
public class SlimeEntityBlue extends SlimeEntityColoured {

    public SlimeEntityBlue(EntityType<? extends SlimeEntityBlue> arg, World arg2) {
        super(arg, arg2);
    }

    @Override
    protected ParticleEffect getParticles() {
        return new ItemStackParticleEffect(ParticleTypes.ITEM, new ItemStack(RegisterItems.SLIME_BALL_BLUE));
    }

    public static final EntityType<SlimeEntityBlue> SLIME_ENTITY_BLUE = Registry.register(
            Registry.ENTITY_TYPE,
            new Identifier(Slimeology.MOD_ID, "slime_entity_blue"),
            FabricEntityTypeBuilder.<SlimeEntityBlue>create(SpawnGroup.MONSTER, SlimeEntityBlue::new)
                    .dimensions(EntityDimensions.changing(2.04f, 2.04f))
                    .trackable(160,4).build());

    public static final EntityType secPointer = SLIME_ENTITY_BLUE;


    // Adapted from SlimeEntityColoured.canSpawnSEC()
    public static boolean canSpawnSECBlue(EntityType<? extends SlimeEntityBlue> type, WorldAccess world, SpawnReason spawnReason,
