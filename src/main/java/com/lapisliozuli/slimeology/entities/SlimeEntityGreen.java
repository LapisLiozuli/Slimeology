...
public class SlimeEntityGreen extends SlimeEntityColoured {

    public SlimeEntityGreen(EntityType<? extends SlimeEntityGreen> arg, World arg2) {
        super(arg, arg2);
    }

    @Override
    protected ParticleEffect getParticles() {
        return new ItemStackParticleEffect(ParticleTypes.ITEM, new ItemStack(RegisterItems.SLIME_BALL_GREEN));
    }

    public static final EntityType<SlimeEntityGreen> SLIME_ENTITY_GREEN = Registry.register(
            Registry.ENTITY_TYPE,
            new Identifier(Slimeology.MOD_ID, "slime_entity_green"),
            FabricEntityTypeBuilder.<SlimeEntityGreen>create(SpawnGroup.MONSTER, SlimeEntityGreen::new)
                    .dimensions(EntityDimensions.changing(2.04f, 2.04f))
                    .trackable(160,4).build());

    public static final EntityType secPointer = SLIME_ENTITY_GREEN;


    // Adapted from SlimeEntityColoured.canSpawnSEC()
    public static boolean canSpawnSECGreen(EntityType<? extends SlimeEntityGreen> type, WorldAccess world, SpawnReason spawnReason,
