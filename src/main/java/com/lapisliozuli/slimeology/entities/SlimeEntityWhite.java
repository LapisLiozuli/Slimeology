...
public class SlimeEntityWhite extends SlimeEntityColoured {

    public SlimeEntityWhite(EntityType<? extends SlimeEntityWhite> arg, World arg2) {
        super(arg, arg2);
    }

    @Override
    protected ParticleEffect getParticles() {
        return new ItemStackParticleEffect(ParticleTypes.ITEM, new ItemStack(RegisterItems.SLIME_BALL_WHITE));
    }

    public static final EntityType<SlimeEntityWhite> SLIME_ENTITY_WHITE = Registry.register(
            Registry.ENTITY_TYPE,
            new Identifier(Slimeology.MOD_ID, "slime_entity_white"),
            FabricEntityTypeBuilder.<SlimeEntityWhite>create(SpawnGroup.MONSTER, SlimeEntityWhite::new)
                    .dimensions(EntityDimensions.changing(2.04f, 2.04f))
                    .trackable(160,4).build());

    public static final EntityType secPointer = SLIME_ENTITY_WHITE;


    // Adapted from SlimeEntityColoured.canSpawnSEC()
    public static boolean canSpawnSECWhite(EntityType<? extends SlimeEntityWhite> type, WorldAccess world, SpawnReason spawnReason,
